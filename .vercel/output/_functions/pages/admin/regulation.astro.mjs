...
const $$Regulation = createComponent(async ($$result, $$props, $$slots) => {
  const { data: items, error } = await supabaseServer.from("regulations").select("*").order("sort_order", { ascending: true });
  ...
  document.querySelectorAll('.delete-btn').forEach(function (btn) {
    btn.addEventListener('click', async function () {
      ...
      try {
        const response = await fetch('/api/regulation/delete', {
          method: 'DELETE',
          headers: {
...
<\/script>`])), renderComponent($$result, "AdminLayout", $$AdminLayout, { "title": "Regulasi" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="mb-8 flex items-center justify-between"> <div> <h1 class="text-3xl font-bold">Regulasi</h1> <p class="mt-2 text-slate-500">Kelola halaman regulasi dan ketentuan.</p> </div> <a href="/admin/regulation/create" class="rounded-xl bg-blue-600 px-5 py-3 text-white hover:bg-blue-700">
Tambah
...
Hapus
</button> </div> </td> </tr>`)} </tbody> </table> ${items?.length === 0 && renderTemplate`<div class="p-8 text-center text-slate-500">Belum ada data regulasi.</div>`} </div> ` }));
}, "D:/Magang Industri/ProjectKKA3/kkakediri/src/pages/admin/regulation.astro", void 0);

const $$file = "D:/Magang Industri/ProjectKKA3/kkakediri/src/pages/admin/regulation.astro";
const $$url = "/admin/regulation";
