...
const POST = async ({ request }) => {
  try {
    ...
    const { error: uploadError } = await supabaseServer.storage.from("about").upload(filename, file);
    if (uploadError) {
    ...
    const { data } = supabaseServer.storage.from("about").getPublicUrl(filename);
