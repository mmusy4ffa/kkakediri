...
const DELETE = async ({ request }) => {
  try {
    ...
    const { data: item, error: getError } = await supabaseServer.from("registrations").select("image").eq("id", id).single();
    if (!getError && item?.image) {
      const fileName = item.image.split("/").pop();
      if (fileName) {
        await supabaseServer.storage.from("registration").remove([fileName]);
      }
    }
